var VQI_PathwayEditor = function(aMode) {
	...
	definitionHub.nodeTypes = {
		nType1 : ["bundleone", "ntype1"],
		nType2 : ["bundletwo", "ntype2"],
		nType3 : ["gene", "ntype3"],
		nType4 : ["geneproduct", "ntype4"],
		nType5 : ["protein", "ntype5"],
		nType6 : ["rna", "ntype6"],
		nType7 : ["microrna", "ntype7"],
		nType8 : ["kinase", "ntype8"],
		nType9 : ["ligand", "ntype9"],
		nType10 : ["receptor", "ntype10"],
		nType11 : ["biologicalprocess", "ntype11"],
		nType12 : ["triangle", "ntype12"],
		nType13 : ["rectangle", "ntype13"],
		nType14 : ["circle", "ntype14"],
		nType15 : ["ellipse", "ntype15"],
		nType16 : ["pentagon", "ntype16"],
		nType17 : ["hexagon", "ntype17"],
		nType18 : ["heptagon", "ntype18"],
		nType19 : ["octagon", "ntype19"],
		nType20 : ["star", "ntype20"],
		nType21 : ["diamond", "ntype21"],
		nType22 : ["vee", "ntype22"],
		nType23 : ["rhomboid", "ntype23"],
		nType24 : ["label", "ntype24"],
		nType25 : ["transcriptionfactor", "ntype25"]
	};
	definitionHub.edgeLineTypes = {
		elType1 : ["solid", "eltype1"],
		elType2 : ["dashed", "eltype2"],
		elType3 : ["dotted", "eltype3"]
	};
	...
	self.NoGUI = function() {
		function sprayColor(lines, obj) {
			var lookup = {};
			for (var i = 0,
			    len = obj.elements.nodes.length; i < len; i++) {
	...
	self.GUI = function(aParent, f) {
		...
		$(function() {
			...
			function addDuplicateNodes(event) {
				...
				for (var i = 0; i < selectedForEditNodes.length; i++) {
					for (var j = 0; j < selectedForEditNodes[i].connectedEdges().length; j++) {
						if (lookupDuplicatedNodes.indexOf("dup" + dupCounter.toString() + selectedForEditNodes[i].connectedEdges()[j].data('source')) != -1 && lookupDuplicatedNodes.indexOf("dup" + dupCounter.toString() + selectedForEditNodes[i].connectedEdges()[j].data('target')) != -1 && lookupDuplicatedEdges.indexOf("dup" + dupCounter.toString() + selectedForEditNodes[i].connectedEdges()[j].data('id')) == -1) {
							edge.push({
								group : "edges",
								data : {
			...
			function findPath(sid, vid) {
				...
				$.post(services['FIND_PATH'], {
					...
					for (var n = 0; n < result.length; n++) {
						...
						count = 1;
						for (var i in resultColumns) {
							var column = resultColumns[i];
							if (column['key'] != "edges") {
								columns.push(row.insertCell(count));
								count++;
								if (n == 0) {
									columns[columns.length - 1].innerHTML = "<i><h5><small>" + column['title'] + "</small></h5></i>";
								} else {
									columns[columns.length - 1].innerHTML = "<h5><small>" + result[n][column['key']] + "</small></h5>";
								}
							}
						}
						console.log(columns)
			...
			self.GUI.refresh = function() {
				if (mode == "test") {
					return {
						...
						pathName : pathName,
						states : states,
						stateRecycle : stateRecycle,
						selectedForQueryNodes : selectedForQueryNodes,
						selectedForEditNodes : selectedForEditNodes,
						selectedForEditEdges : selectedForEditEdges,
			...
			self.GUI.expose = function() {
				if (mode == "test") {
					return {
						removeHeroUnit : removeHeroUnit,
						newPathway : newPathway,
						focus : focus,
						unFocus : unFocus,
						search : search,
						exitSearch : exitSearch,
						onChangePathwayFile : onChangePathwayFile,
						collapseBundleInformative : collapseBundleInformative,
						expandBundleInformative : expandBundleInformative,
						collapseBundle : collapseBundle,
						expandBundle : expandBundle,
						onChangeColoringFile : onChangeColoringFile,
						postAddProcessing : postAddProcessing,
						define : define,
						setElements : setElements,
						loadPathway : loadPathway,
						onSelect : onSelect,
						saveAsPathway : saveAsPathway,
						savePathway : savePathway,
						onPathwayReaderLoad : onPathwayReaderLoad,
						onColoringReaderLoad : onColoringReaderLoad,
						sprayColor : sprayColor,
						removeElements : removeElements,
						addDuplicateNodes : addDuplicateNodes,
						addNode : addNode,
						addEdge : addEdge,
						unbundleRecursive : unbundleRecursive,
						recursiveBundle : recursiveBundle,
						unbundle : unbundle,
						bundle : bundle,
						mapForExport : mapForExport,
						produceJSON : produceJSON,
						refreshPathwayList : refreshPathwayList,
						findPath : findPath,
						findPathsAll : findPathsAll,
						findPathOne : findPathOne,
						saveState : saveState,
						undo : undo,
						redo : redo,
						clone : clone,
						editEdgeArrowType : editEdgeArrowType,
						editEdgeLineType : editEdgeLineType,
						editEdgeDirection : editEdgeDirection,
						toggleEdgeStyle : toggleEdgeStyle,
						edgeEnableSegmentedStyle : edgeEnableSegmentedStyle,
						edgeDisableSegmentedStyle : edgeDisableSegmentedStyle,
						moveElementtoBackground : moveElementtoBackground,
						moveElementtoForeground : moveElementtoForeground,
						removeBackgroundImageOnNode : removeBackgroundImageOnNode,
						onChangeBackgroundImageOnNode : onChangeBackgroundImageOnNode,
						editNodeWidth : editNodeWidth,
						editNodeHeight : editNodeHeight,
						editNodeType : editNodeType,
						editNodeName : editNodeName,
						editNodeMut : editNodeMut,
						editNodeCnv : editNodeCnv,
						editNodeRna : editNodeRna,
